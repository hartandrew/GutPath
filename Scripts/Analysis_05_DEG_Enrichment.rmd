...
# Summarize DEGs per condition × cell type
summary_df <- sig_degs %>%
...
table(Ileum_filt$InfectionStatus,Ileum_filt$Mouse, Ileum_filt$secondlevel)
unique(Ileum_filt$secondlevel)
meta <- Ileum_filt@meta.data
meta_subset <- meta %>%
 dplyr::select(InfectionStatus, Mouse, secondlevel, FineCellType, SampleType,
...
```{r}
# Replot the heatmaps according to mouse replicate 
meta_subset <- meta_subset %>%
  mutate(across(all_of(pathways_interest), as.numeric))

meta_subset$sample <- paste0(meta_subset$InfectionStatus, "_", meta_subset$Mouse)
heat <- list()
for (i in pathways_interest) {
  
  avg_scores <- meta_subset %>%
  group_by(sample, secondlevel) %>%
  summarise(mean_score = mean(!!sym(i), na.rm = TRUE), .groups = "drop")
...
#avg_scores$InfectionStatus <- factor(avg_scores_scaled$InfectionStatus, levels = infectionorder)
heat[[i]] <- ggplot(avg_scores_scaled, aes(x = secondlevel, y = sample, fill = scaled_score)) +
  geom_tile(color =NA) +
  scale_fill_gradient2(
    low = "#1C75BC", mid = "grey", high = "#F7941D",
    midpoint = midpoint,
...
heat[["HALLMARK_INTERFERON_GAMMA_RESPONSE"]] + theme( title = element_blank())
ggsave("Hallmark_IFNG_Ucell_Heatmap_AllSamples_MouseReps.svg", plot = last_plot() ,device = NULL,
  path = images, scale = 1, width = 11, height = 5, limitsize = TRUE, bg = NULL)
heat[["HALLMARK_TGF_BETA_SIGNALING"]] + theme( title = element_blank())
ggsave("Hallmark_TGFb_Ucell_Heatmap_AllSamples_MouseReps.svg", plot = last_plot() ,device = NULL,
  path = images, scale = 1, width = 11, height = 5, limitsize = TRUE, bg = NULL)
...
# graph by Mouse sample to show that replicates are consistent

meta <- Ileum_filt@meta.data %>%
  dplyr::select(Mouse, InfectionStatus, secondlevel, SampleType)
unique(meta$SampleType)
meta <- meta[meta$SampleType %in% c("Ileum Lamina Propria","Ileum Intestinal Epithelial Cells" ),]
meta$sample <- paste0(meta$InfectionStatus, "_", meta$Mouse)
freq_data <- meta %>%
  group_by(sample, secondlevel) %>%
  summarise(n = n(), .groups = "drop") %>%
  group_by(sample) %>%
...
freq_data$InfectionStatus <- sub("_.*", "", freq_data$sample)
cell_types <- c("Monocytes", "Goblet Cells", "Neutrophils", "Tuft Cells")

plots <- lapply(cell_types, function(ct) {
  ggplot(filter(freq_data, secondlevel == ct),
         aes(x = sample, y = freq, fill = InfectionStatus)) +
    geom_bar(stat = "identity", position = "dodge") +
    labs(
      title = ct,
      y = "Fraction of cells",
...
plots[[3]] + theme(axis.title.x = element_blank(), 
                   plot.title = element_blank(),
                   axis.text = element_text(color = "black"))
ggsave("Neutrophils_frequency_Ileum_By_Infection_MouseReps.svg", plot = last_plot(), path = images, width = 6, height = 4)

plots[[4]] + theme(axis.title.x = element_blank(), 
                   plot.title = element_blank(),
                   axis.text = element_text(color = "black"))
ggsave("TuftCells_frequency_Ileum_By_Infection_MouseReps.svg", plot = last_plot(), path = images, width = 6, height = 4)
